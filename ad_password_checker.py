...
class PasswordExpiryTable(QTableWidget):
    ...
    def populate_data(self, users_data: List[UserPasswordInfo]):
        ...
        for row, user in enumerate(users_data):
            ...
            # Days until expiry
            days_item = QTableWidgetItem(str(user.days_until_expiry))
            if user.password_never_expires:
                days_item.setText("Never")
            elif user.days_until_expiry < 0:
                days_item.setText(f"{user.days_until_expiry}")
...
class ConfigurationWidget(QWidget):
    ...
    def load_config_values(self):
        ...
        self.server_edit.setText(self.config.config.get("server", ""))
